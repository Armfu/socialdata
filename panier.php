...
if(isset($_POST['payer']))
{
	for($i=0 ;$i < count($_SESSION['panier']['id_produit']) ; $i++) 
	{
		...
		if($produit['stock'] < $_SESSION['panier']['quantite'][$i])
		{
			...
			if($produit['stock'] > 0)
			{
				$contenu .= '<div class="erreur">la quantité de produit ' . $_SESSION['panier']['id_produit'][$i] . ' à été réduite car notre stock était insuffisant, veuillez vérifier vos achats.</div>';
				$_SESSION['panier']['quantite'][$i] = $produit['stock'];
			}
			else
			{
				$contenu .= '<div class="erreur">le produit ' . $_SESSION['panier']['id_produit'][$i] . ' à été retiré de votre panier car nous sommes en rupture de stock, veuillez vérifier vos achats.</div>';
...
echo "<tr><th>Titre</th><th>Produit</th><th>Quantité</th><th>Prix Unitaire</th><th>Prix Totaux</th></tr>";
...
echo "</table><br />";
echo "<i>Réglement par moyen ectronique uniquement : MOBICASH 1016192</i><br />";
